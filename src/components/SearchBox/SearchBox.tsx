...
export default function SearchBox() {
  ...
  const [tripType, setTripType] = useState<string>("roundWay");
  const [isOne, setIsOne] = useState<boolean>(tripType === "oneWay");
  const [isMulti, setIsMulti] = useState<boolean>(tripType === "multiWay");
  const handleTripTypeChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const selectedValue = (event.target as HTMLInputElement).value;
    setTripType(selectedValue);
    setIsOne(selectedValue === "oneWay");
    setIsMulti(selectedValue === "multiWay");
  };
  ...
  return (
    <div className=" px-6 rounded-[16px] relative z-50 bg-white py-5">
        ...
        <FormControl component="fieldset">
          ...
          >
            <FormControlLabel
              value="oneWay"
              control={<Radio />}
              label={<span className="text-[16px] font-medium">One way</span>}
            />
            <FormControlLabel
              value="roundWay"
              control={<Radio />}
              label={<span className="text-[16px] font-medium">Round Way</span>}
            />
            <FormControlLabel
              value="multiWay"
              control={<Radio />}
              label={<span className="text-[16px] font-medium">Multi Way</span>}
            />
          </RadioGroup>
        </FormControl>
        <div>
      ...
      <div className="rounded-[9px] items-center border border-[#808285]  grid grid-cols-12 px-5">
        <div
          className={`grid w-full transition-width transition-transform duration-500 grid-cols-8 ${isOne ? "col-span-6" : "col-span-5"}`}
        >
        ...
        <div
          className={`flex w-full flex-row items-center ${isOne ? "col-span-6" : "col-span-7"}`}
        >
          <div className="border-l w-full border-gray-400 pl-5">
            <DateSelectContainer
              setIsOne={setIsOne}
              isOne={isOne}
              setDatesArr={setDatesArrLocal}
            />
          </div>
      ...
      {isMulti ? (
        <div className="w-full  grid grid-cols-10">
          <div className="rounded-[9px] w-full col-span-7  items-center border mt-4 border-[#808285]  grid grid-cols-11 px-5">
            <div
              className={`grid w-full transition-width transition-transform duration-500 grid-cols-8 ${isOne ? "col-span-8" : "col-span-7"}`}
            >
              <div className="col-span-3">
                <SearchInput
                  title={"From"}
            ...
            <div
              className={`flex w-full flex-row items-center ${isOne ? "col-span-3" : "col-span-4"}`}
            >
              <div className="border-l w-full border-gray-400 pl-5">
                <DateSelectContainer
                  setIsOne={setIsOne}
          ...
          <FormControl component="fieldset">
            ...
            >
              <FormControlLabel
                ...
                label={
                  <span className="text-[14px] font-normal">Regular Fare</span>
                }
              />
              <FormControlLabel
                value="studentFare"
                control={<Radio size="small" />}
                label={
                  <span className="text-[14px] font-normal">Student Fare</span>
                }
              />
            </RadioGroup>
          </FormControl>
